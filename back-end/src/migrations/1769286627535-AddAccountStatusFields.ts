...
export class AddAccountStatusFields1769286627535 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`username_hash\` char(64) NOT NULL`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`idx_username_hash\` ON \`users\` (\`username_hash\`)`);
        // Add role column
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`role\` varchar(50) NOT NULL DEFAULT 'USER'`);
        // Add status column as VARCHAR (not ENUM)
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`status\` varchar(50) NOT NULL DEFAULT 'ACTIVE'`);
        ...
        await queryRunner.query(`ALTER TABLE \`relay_emails\` CHANGE \`deleted_at\` \`paused_at\` datetime(6) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`users\` CHANGE \`last_logined_at\` \`last_logined_at\` datetime(6) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`relay_emails\` CHANGE \`paused_at\` \`deleted_at\` datetime(0) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`username_hash\``);
